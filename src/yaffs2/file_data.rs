...
use crate::yaffs2::{config::Config, object_header::Header};
...
impl FileData {
    pub fn new(object_id: INodeNo, config: Config) -> Self {
        Self {
            object_id,
            header: None,
            chunks: DashMap::new(),
    ...
    pub fn is_complete(&self) -> bool {
        if let Some(header) = &self.header {
            let expected_chunks =
                (header.size as usize + self.config.page_size - 1) / self.config.page_size;
